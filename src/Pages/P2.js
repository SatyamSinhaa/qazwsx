    ...
    return (
        <div>
            ...
            {images.slice(1, counter).map((image, index) => (
                <div>
                    <img key={index} src={image.image} alt={`pic ${index + 2}`} height={500} />
                    <h3 style={{color:'black'}}>{image.line}</h3>
                </div>
            ))}

            {allert && <div style={{color:'red'}}>are, vo button hena usko daba na</div>}
            <br/><button onClick={handleCounter}>click</button>
            {displayButton && <button onClick={() => navigate("/p3")}>next page</button>}
        </div>
    );
}
